...
3.How can we make a plot look more professional? 
- take it as minimal as possible, no "junk", no Color, if no color is needed, scale comprehensible
...
-   Increases contrast
-   Allows for consistency (same country with the same color)
...
-   Represent categories (not more than 4 colors)
-   Represent values:
  -   Only if necessary (i.e. you are using the x and y axis for more important variables)
  -   Not accurate (show trends)
...
In R, grammar of graphics is implemented in ```ggplot()```, a function in the
```ggplot2``` package.
...
```ggplot()``` is the *function* to plot
```aes``` or *astehtic mappings* is the logical connection bewtween your data and the plot element
```geom``` defines the type of plot like 
...
```
Overview ggplot aesthetics:
[https://ggplot2.tidyverse.org/reference/index.html#section-aesthetics](https://ggplot2.tidyverse.org/reference/index.html#section-aesthetics)

Overview ggplot geometrics:
[https://ggplot2.tidyverse.org/reference/index.html#section-geoms](https://ggplot2.tidyverse.org/reference/index.html#section-geoms)
